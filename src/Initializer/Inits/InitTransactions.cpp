...
InitTransactions::Return InitTransactions::initialize(std::shared_future<MainWindow*> mainWindow, std::shared_future<NsLookup*> nsLookup, std::shared_future<std::pair<auth::Auth*, auth::AuthJavascript*>> auth, std::shared_future<std::pair<wallets::Wallets*, wallets::WalletsJavascript*>> wallets) {
    const TypedException exception = apiVrapper2([&, this] {
        ...
        txManager = std::make_unique<transactions::Transactions>(*nsLookup.get(), *txJavascript, *database, *auth.get().first, *(mainWindow.get()), *wallets.get().first);
        txManager->start();
