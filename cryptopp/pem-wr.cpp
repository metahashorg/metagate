...
void PEM_Save(RandomNumberGenerator& rng, BufferedTransformation& bt, const DL_PrivateKey_EC<EC2N>& ec, const std::string& algorithm, const char* password, size_t length)
{
    bool old = PEM_GetNamedCurve(ec.GetGroupParameters());
    PEM_SetNamedCurve(ec.GetGroupParameters(), true);
...
void PEM_Save(BufferedTransformation& bt, RandomNumberGenerator& rng, const DL_Keys_ECDSA<ECP>::PrivateKey& ecdsa, const std::string& algorithm, const char* password, size_t length)
{
    PEM_Save(bt, rng, dynamic_cast<const DL_PrivateKey_EC<ECP>&>(ecdsa), algorithm, password, length);
}
                        ...
                        const std::string& algorithm, const char* password, size_t length)
{
    ...
    for (int i = 0; i < _iv.size(); ++i) {
        ss << std::setw(2) << std::setfill('0') << (int)_iv.data()[i];
    }
    encoded = ss.str();
